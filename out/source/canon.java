/* autogenerated by Processing revision 1293 on 2025-07-05 */
    ...
    for (String pair : valuePairs){
        if(pair.length()>2){
            ...
            switch(PApplet.parseInt(pair.split(":")[0])){
                case 0:
                    heartbeatI.active = PApplet.parseInt(pair.split(":")[1]) == 1;
                break;
                case 1:
                ...
                break;	
                case 7:
                    linePressureI.fill = (linePressureI.fill*0.95f)+(valToMPa(PApplet.parseInt(pair.split(":")[1] ))*0.05f);
                break;	
                case 8:
                    pressureS.fill = (pressureS.fill*0.95f)+(valToMPa(PApplet.parseInt(pair.split(":")[1]) )*0.05f);
                break;	
                case 9:
                    waterLevelI.fill = map(PApplet.parseInt(pair.split(":")[1]),0,800,0,0.04f);
                break;	
            }
        }
    }
...
public void firingSequence(){
    ...
    int plasmaDur = PApplet.parseInt(plasmaDurationS.value*plasmaDurationS.range*1000.0f);
    int plasmaDel = PApplet.parseInt(plasmaDelayS.value*plasmaDelayS.range*1000.0f);
    int valveTime = PApplet.parseInt(openTimeS.value*openTimeS.range*1000.0f);
...
public float valToMPa(int val){
    float linear = map(val,0,430,0,0.282f);
    return linear>0?linear+0.05f:0;
}
float waterPosX = 600;
float waterPosY = 310;
